    ...
    <div>
        <form method="post" action="${pageContext.request.contextPath}/cars/add">
            <div class="form-group">
                ...
                <input class="form-control" id="model" name="model" type="text" required="required">
            </div>
            <div class="form-group">
                <select name="option_manufacturer" style="width: 20%">
                    <option>Select Manufacturer</option>
                    <c:forEach var="op_manufacturer" items="${manufacturerList}">
                        <option value="${op_manufacturer.id}"><c:out value="${op_manufacturer.id}"/> -- <c:out
                                value="${op_manufacturer.name}"/></option>
                    </c:forEach>
                </select><br>
            </div>
            <br/>
